...
// Adding event listener for "Get Started" button to switch views
...
// Function to create weather card for current and future forecast
...
function gettingWeatherDetails(cityWeather, lat, lon) {
    ...
    fetch(weather_api_url).then(res => res.json()).then(data => {
        ...
        // Populate weather details and forecast
        ...
        // Initialize Swiper after data is inserted
        forecastSwipe();

    }).catch(() => {
        alert('Error Occurred While Fetching the Weather Data');
    });
}

// Search button click event to fetch weather
    ...
    if (cityName === "") {
        ...
        fetch(geocoding_api_url).then(res => res.json()).then(data => {
            if (!data.length) {
                ...
                const { name, lat, lon } = data[0]; // Store the name, latitude, and longitude
                gettingWeatherDetails(name, lat, lon); // Call function to fetch weather details
            }
        }).catch(() => {
